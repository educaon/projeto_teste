...
def analise_shapiro(dataframe, alfa=0.05):
    ...
    print("Tesde de Shapiro-Wilk")
    
    for coluna in dataframe.columns:
        estatistica_shapiro, valor_p_shapiro = shapiro(dataframe[coluna], nan_policy="omit")
        print(f"{estatistica_shapiro=:.3f}")
        if valor_p_shapiro > alfa:
            print(f"{coluna} segue uma distribuição normal (valor p: {valor_p_shapiro:.3f})")
        else:
            print(f"{coluna} NÃO segue uma distribuição normal (valor p: {valor_p_shapiro:.3f})")
